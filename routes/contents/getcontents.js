...
router.post('/', async (req, res) => {
   ...
   if(!contentsIdx||!userIdx||!contentsType) {
      ...
      if(contentsType==0){
      let selectImageQuery = `SELECT C.contentsIdx, C.contentsTitle, C.contentsInfo, C.contentsCategory, C.contentsDate,C.thumbnailUrl,
                                          ...
                                          (SELECT I.imgUrl FROM Imerge as I WHERE I.contentsIdx = C.contentsIdx and Imerge.imgname=I.imgname) as contentsUrl,
        ...
        if(!selectImageResult){
          ...
          let selectVideoQuery = `SELECT C.contentsIdx, C.contentsTitle, C.contentsInfo, C.contentsCategory, C.contentsDate, C.thumbnailUrl,
                                          C.contentsLike,hashName1, hashName2, hashName3,
                                          (SELECT count(ccmtIdx) FROM Ccomment WHERE Ccomment.contentsIdx = C.contentsIdx) as commentCnt,
                                          CASE
                            WHEN
                                (SELECT 
